    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="font-black text-2xl mb-4" style={{ color: "#0A0A0A" }}>Статья не найдена</h1>
          ...
          <div className="lg:col-span-2">
            <div className="prose max-w-none" style={{ color: "#444" }}>
              {paragraphs.map((para, i) => {
                if (para.startsWith("## ")) {
                  return (
                    <h2 key={i} className="font-black text-xl mt-8 mb-4" style={{ color: "#0A0A0A", letterSpacing: "-0.02em" }}>
                      {para.replace("## ", "")}
                    </h2>
                  );
                }
                if (para.startsWith("**") && para.endsWith("**")) {
                  return (
                    <p key={i} className="font-bold text-sm mb-3" style={{ color: "#0A0A0A" }}>
                      {para.replace(/\*\*/g, "")}
                    </p>
                  );
                }
              ...
              <div className="space-y-4">
                {articles.filter((a) => a.slug !== slug).slice(0, 3).map((a) => (
                  <Link key={a.slug} href={`/blog/${a.slug}`} className="block no-underline group">
                    ...
                    <div className="text-sm font-semibold group-hover:underline" style={{ color: "#0A0A0A" }}>
                      {a.title}
                    </div>
                  </Link>
                ))}
            ...
            <Link key={article.slug} href={`/blog/${article.slug}`} className="no-underline group">
              <div className="bg-white p-8 h-full" style={{ transition: "background 0.15s" }}>
                ...
                <h2
                  ...
                  style={{ color: "#0A0A0A", letterSpacing: "-0.02em", lineHeight: 1.2 }}
                >
                  {article.title}
                </h2>
